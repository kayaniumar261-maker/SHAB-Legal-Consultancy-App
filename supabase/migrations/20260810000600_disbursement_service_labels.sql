...
-- Normalize the earlier generic category without changing historical amounts.
update public.expenses
set category = 'Translation Charges',
    updated_at = now()
where lower(btrim(category)) = 'translation';

-- The invoice service title is the exact disbursement category. Supporting
-- details remain in notes for a clear, auditable client document.
update public.invoices invoice
set description = expense.category,
    notes = expense.description ||
      '. Client disbursement dated ' || to_char(expense.expense_date, 'DD Mon YYYY') ||
...
begin
  ...
  insert into public.invoices (
    ...
    target.client_id, target.case_id, '', p_issue_date, p_due_date, 'draft',
    target.currency, target.total_amount, 0, 0, 0, target.total_amount, 0, 0,
    target.total_amount,
    target.category,
    target.description || '. Client disbursement dated ' ||
      to_char(target.expense_date, 'DD Mon YYYY') || '. Supplier: ' ||
      coalesce(target.vendor_name, 'Not specified') || '. Expense reference: ' ||
      target.expense_number || '.',
    auth.uid(), target.total_amount, 'out_of_scope', p_issue_date, false
